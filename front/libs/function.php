...
function fileinclude($filename, $fileType = 'html', $mod_tb_about_masterkey, $for = 'check', $crop = false, $cropthumb = false)
{
    ...
    if (file_exists($checkFile) && $filename) {
        ...
        if ($mod_tb_about_masterkey == 'cu') {
            $setimg = "/public/img/asset/default_boss.png";
			
        } else  if ($mod_tb_about_masterkey == 'dt') {
            $setimg = "/assets/img/icon/km-doc.svg";
        } else {
            // $setimg = "/assets/img/icon/none-img1.png";
            $setimg = "/assets/img/icon/none-img2.jpg";
        }
    }
...
function loadSendEmailTo($mailTo, $mailFrom = null, $subjectMail = null, $messageMail = null, $typeMail = 1)
{
    /* ############################################### */
    if ($typeMail == 1) {
...
function getNameinTable($GID, $tableDb)
{
    ############################################
    global $config, $db, $url;
    $sql = "SELECT
    ...
    $txt_member_name = $result->fields['subject'];
    $textReturn = $txt_member_name;

    return $textReturn;
}

function page_redirect($masterkey, $action, $id, $condition = null){
    $param = $action . "|" . $masterkey ."|" . $id;
    if (!empty($condition)) {
       $param .= "|".$condition;
    }
    return encodeStr($param);
 }

 function path_url($param) {
    global $path_root;
 
    if (!empty($path_root)) {
       $param = str_replace($path_root, '', $param);
       $url_explde = array_filter(explode("/", $param));
       
       if (!in_array('https:', $url_explde) && !in_array('http:', $url_explde)) {
          $param = "";
