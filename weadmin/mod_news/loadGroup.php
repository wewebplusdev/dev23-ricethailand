...
<body>
    <?php
    // SQL SELECT PERMISSION Filter#########################
    $callGauthen = "Select
" . $mod_tb_permisGroup . "." . $mod_tb_permisGroup . "_misid as idG
From
" . $mod_tb_permisGroup . "
Where
" . $mod_tb_permisGroup . "." . $mod_tb_permisGroup . "_misid = '" . $_SESSION[$valSiteManage . "core_session_agency"] . "'
AND " . $mod_tb_permisGroup . "." . $mod_tb_permisGroup . "_masterkey = '" . $_REQUEST['masterkey'] . "'";
    $listAuthenAgen = $dbConnect->execute($callGauthen);
    $listGAllowAgen = array();
    foreach ($listAuthenAgen as $key => $value) {
        $listGAllowAgen[] = $value['idG'];
    }
    // print_pre($sql1);
    ...
    <form action="?" method="post" name="myForm" id="myForm">
        ...
        <div class="divRightHeadSearch">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding-top:20px;" align="center">
                ...
                <!-- <tr>
                    <td class="">
                        <select name="inputgroupID" id="inputgroupID" class="formSelectSearchStyle">
                            <option value=""><?php echo  "เลือก" . $langMod["tit:selectag"] ?> </option>
                            <?php
                            $sql_group = "SELECT " . $mod_tb_root_group . "_id," . $mod_tb_root_group . "_subject FROM " . $mod_tb_root_group . "
                             WHERE  " . $mod_tb_root_group . "_status ='Enable' AND " . $mod_tb_root_group . "_masterkey ='" . $masterkeyAgen . "'   ";
                            $sqlChecklistAgen = array();
                            foreach ($listGAllowAgen as $idGroup) {
                                $sqlChecklistAgen[] = $mod_tb_root_group . "_id = $idGroup ";
                            }
                            if ($_SESSION[$valSiteManage . 'core_session_level'] != "admin") {


                                $sql_group .= " and ( " . implode(" or ", $sqlChecklistAgen) . ") ";
                            }
                            ...
                            ?>
                            <option value="<?php echo  $row_groupid ?>" <?php if ($_REQUEST['inputgroupID'] == $row_groupid) { ?> selected="selected"
                ...
                <!-- <td class="selectSearch1">
                        <select name="inputStatus" id="inputStatus" class="formSelectSearchStyle">
                            <option value="">เลือกสถานะ</option>
                            <option value="Enable" <?php if ($_REQUEST['inputStatus'] == 'Enable') { ?>selected="selected"
                                <?php  } ?>>Enable
                            </option>
                            <option value="Home" <?php if ($_REQUEST['inputStatus'] == 'Home') { ?>selected="selected"
                                <?php  } ?>>Home
                            </option>
                            <option value="Disable" <?php if ($_REQUEST['inputStatus'] == 'Disable') { ?>selected="selected"
                                <?php  } ?>>Disable
                            </option>
                        </select>
                    </td> -->
            ...
            <script>
                $(document).on('change', '#inputgroupID', function() {

                    $('#inputStatus option:first-child').attr("selected", "selected");
                    $('#myForm').submit();
                });

                $(document).on('change', '#inputStatus', function() {
                    $('#inputgroupID option:first-child').attr("selected", "selected");
                    $('#myForm').submit();
                });
            </script>
        </div>
        ...
        <div class="divRightMain">
            ...
            <table width="96%" border="0" cellspacing="0" cellpadding="0" align="center" class="tbBoxListwBorder">
                <tr>
                    ...
                    <td align="left" valign="middle" class="divRightTitleTb"><span class="fontTitlTbRight"><?php echo  $langMod["tit:subjectg"] ?>
                    ...
                    <?php if ($_SESSION[$valSiteManage . 'core_session_languageT'] == 2) { ?>
                        <td width="22%" align="left" valign="middle" class="divRightTitleTb"><span class="fontTitlTbRight"><?php echo  $langMod["tit:subjectg"] ?>
...
" . $mod_tb_root_group . "_pic
 ...
 FROM " . $mod_tb_root_group;
                ...
                if ($inputgroupID != "") {
                    $sql = $sql . "  AND    " . $mod_tb_root_group . "_agenid='" . $inputgroupID . "'  ";
                }

                if ($inputStatus != '') {
                    $sql = $sql . "  AND    " . $mod_tb_root_group . "_status='" . $inputStatus . "'  ";
                }
                                        ...
                                        <td align="left">
                                            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                                <tr>
                                                    ...
                                                    <td align="left" style="padding-left:10px; " valign="center">
                            ...
                            <td class="divRightContantOverTbR" valign="top" align="center">
                                <?php if ($valPermission == "RW") { ?>
                                    <table border="0" cellspacing="0" cellpadding="0">
                                        <tr>
                                            <?php if ($_REQUEST['inputgroupID'] != 0 || true) { ?>
