...
<body>
	...
	if (!empty($_FILES['inputFileUpload']['error'])) {
		...
		$getTypeAllow = getTypeAllow($core_allow_file_masterkey);
		$type_file = pathinfo($_FILES['inputFileUpload']['name'], PATHINFO_EXTENSION);
		if (in_array($type_file, $getTypeAllow)) {
			...
			$sql = "SELECT " . $mod_tb_fileTemp . "_id," . $mod_tb_fileTemp . "_filename," . $mod_tb_fileTemp . "_name FROM " . $mod_tb_fileTemp . " WHERE " . $mod_tb_fileTemp . "_contantid 	='" . $_REQUEST['myID'] . "'  AND   " . $mod_tb_fileTemp . "_language ='" . $_REQUEST['langt'] . "'  ORDER BY " . $mod_tb_fileTemp . "_id ASC";
