...
function logs_access($action, $actionType = null)
{
    ########################################################################################
...
function getUserPermissionOnMenu($myUserID, $myMenuID = null)
{
    ############################################
...
function resize($img= null, $w = null, $h = null, $newfilename = null)
{
    ############################################
...
function getSystemLangTxt($langVal, $thaiVal = null, $engVal = null)
{
    ############################################
...
//#################################################
function ADDSTAMP($add, $to = null, $msg = null, $count = null, $gid = null)
{
    //#################################################
...
{
    //#################################################
    global $core_session_language, $valSiteManage;
    if ($DateTime == "") {
        $DateTime = "00-00-0000";
    }
...
function loadNameCityIDserviceType($valCityID, $valTypeServ = null)
...
function loadNameCityName($valCityID, $valTypeServ = null)
...
function loadNameRegionIDserviceType($valRegionID, $valTypeServ = null)
...
function loadNameRegionName($valRegionID, $valTypeServ = null)
...
function loadNameHotelName($valHotelID, $valTypeServ = null)
{ // หาชื่อ Region ของ Database
    ############################################
    global $coreLanguageSQL;
...
function loadNameRoomName($valRoomID, $valTypeServ = null)
{ // หาชื่อ Region ของ Database
    ############################################
    global $coreLanguageSQL;
...
function loadSendEmailTo($mailTo, $mailFrom = null, $subjectMail = null, $messageMail = null)
{
    ...
    /* ############################################### */
    // Google OAuth2
    require_once str_replace("\\", '/', dirname(__FILE__)) . '/sendEmailPHP7.php';
...
function dateCalAddDay($valDate, $valAdd = null)
...
function getMasterIDRegister($masterkey, $valPath = null)
{ // หาชื่อ Room ของ Database
    ############################################
    global $coreLanguageSQL;
...
function dateDivMin($t1, $t2 = null)
{ // ส่งวันที่ที่ต้องการเปรียบเทียบ ในรูปแบบ มาตรฐาน 2006-03-27 21:39:12
    ############################################
    $t1Arr = splitTime($t1);
    $t2Arr = splitTime($t2);
...
//#################################################
function ranDomStr($length, $id = null)
{
    //#################################################
...
function rePTxt($txt, $txtChange = null, $type = null, $txtTo = null)
{
...
###########################################
function loadGetURLByMod($valWebsite, $valLang = null, $valFolder = null, $valID = null, $type = null)
{ // หา ID Hotel ของ Webservices
    ############################################
...
############################################
function loadGetURLMinisiteByType($valTypeUrl, $valMinisite = null, $valWebsite = null)
{ // หา ID Hotel ของ Webservices
    ############################################
    if ($valTypeUrl == 1) {
...
############################################
function getNameGroupTable($GID, $tableDb = null)
{
    ############################################
    global $coreLanguageSQL;
    $sql = "SELECT
...
function getIDTable($GID, $tableDb = null)
{
    ############################################
    global $coreLanguageSQL;
    $sql = "SELECT
    ...
    $txt_member_name = changeQuot($row[2]);
    $textReturn = $txt_member_name;

    return $textReturn;
}
############################################
function getNameGroupTableProvince($GID, $tableDb = null)
{
    ############################################
    global $coreLanguageSQL;
    $sql = "SELECT
...
############################################
function getNameGroupTableIDArray($arrID, $tableDb = null)
{
    ############################################
    $sql = "SELECT
    " . $tableDb . "_id ,
...
############################################
function getNameGroupTablePDSDArray($arrID, $tableDb = null)
{
    global $dbConnect;
    ############################################
    $sql = "SELECT
...
############################################
function str_replaceExport($valSql, $typeReplaca = null)
{
    ############################################
    $valReplaceBefore = " ";
    $valReplaceAfter = "|x|";
...
/* START REGISTER 20/7/2021 */
function getpathCoreWeadmin($res = null){
    $default = explode('/',$res);
    // return $default[1]."/".$default[2]."/".$default[3];
    return $default[1]."/".$default[2];
}
/* END REGISTER 20/7/2021 */

function getRreplaceForView($dataLang = null){
	foreach ($dataLang as $keyLang => $valueLang ) {
		$langMod[$keyLang] = str_replace("โปรดป้อน","ข้อมูล",$valueLang);
	}
...
function getTextFileUploadVideo($dataLang = null){
    global $dbConnect, $coreLanguageSQL;
	...
	$sql = "SELECT
	" . $tableDb . "_subject as subject
	FROM " . $tableDb . " WHERE   " . $tableDb . "_masterkey 	= '" . $keyDb . "'  AND  " . $tableDb . "_status 	= 'Enable' ORDER BY   " . $tableDb . "_order  DESC ";
	$query = wewebQueryDB($coreLanguageSQL, $sql);
	
	while($row = wewebFetchArrayDB($coreLanguageSQL, $query)) {
		$textVideoFile .=  ".".$row['subject'].", ";
		$textVideoFileAc .=  "video/".$row['subject'].",";
	}
	
	$textVideoFile = substr($textVideoFile,0,-2);
	$textVideoFileAc = substr($textVideoFileAc,0,-1);

	foreach ($dataLang as $keyLang => $valueLang ) {
		$langMod[$keyLang] = str_replace("|weweb_video|",$textVideoFile,$valueLang);
	}
	
	$langMod['dev0821:videofile'] = $textVideoFileAc;
	
	$langMod = getTextFileUploadAtt($langMod);
	
	return $langMod;
	
}

function getTextFileUploadAtt($dataLang = null){
    global $dbConnect ,$coreLanguageSQL;
	...
	foreach ($dataLang as $keyLang => $valueLang ) {
		$langMod[$keyLang] = str_replace("|weweb_file|",$textVideoFile,$valueLang);
	}
	
	$langMod['dev0821:attfile'] = $textVideoFileAc;

	$langMod = getTextFileUploadPic($langMod);
	
	return $langMod;
	
}

function getTextFileUploadPic($dataLang = null){
    global $dbConnect ,$coreLanguageSQL;
	...
	foreach ($dataLang as $keyLang => $valueLang ) {
		$langMod[$keyLang] = str_replace("|weweb_pic|",$textVideoFile,$valueLang);
	}
	
	$langMod['dev0821:picfile'] = $textVideoFileAc;
	
	return $langMod;
	
}

function getTypeAllow($masterkey){
    global $dbConnect ,$coreLanguageSQL;

	$tableDb  = "md_cmg";
	$sql = "SELECT
	" . $tableDb . "_subject as subject
	FROM " . $tableDb . " WHERE   " . $tableDb . "_masterkey 	= '" . $masterkey . "'  AND  " . $tableDb . "_status 	= 'Enable' ORDER BY   " . $tableDb . "_order  DESC ";
	$query = wewebQueryDB($coreLanguageSQL, $sql);
    $array_result = array();
    foreach ($query as $key => $value) {
        $array_result[] = $value['subject'];
    }
