...
class ADODB_mysql extends ADOConnection {
	...
	function MetaDatabases()
	{
		...
		$arr = array();
		$i = 0;
		$max = wewebNumRowsDB($coreLanguageSQL,$qid);
		while ($i < $max) {
...
//		return "from_unixtime(unix_timestamp($date)+$fraction)";
	...
	return wewebQueryDB($coreLanguageSQL, $sql,$this->_connectionID);
	/*
...
class ADORecordSet_mysql extends ADORecordSet{
	...
	{
	//GLOBAL $ADODB_COUNTRECS;
	//	$this->_numOfRows = ($ADODB_COUNTRECS) ? @wewebNumRowsDB($coreLanguageSQL,$this->_queryID):-1;
		$this->_numOfRows = @wewebNumRowsDB($coreLanguageSQL,$this->_queryID);
	...
	function MoveNext()
	{
		...
		if (@$this->fields = wewebFetchArrayDB($coreLanguageSQL,$this->_queryID,$this->fetchMode)) {
			$this->_updatefields();
			$this->_currentRow += 1;
			return true;
		}
	...
	function _fetch()
	{
		$this->fields = @wewebFetchArrayDB($coreLanguageSQL,$this->_queryID,$this->fetchMode);
		$this->_updatefields();
