...
- Added an intermediate variable for mysql _fetch() and MoveNext() to store fields, to prevent overwriting field array with boolean when wewebFetchArrayDB($coreLanguageSQL,) returns false.
