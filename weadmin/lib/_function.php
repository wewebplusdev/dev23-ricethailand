...
############################################
    global $coreLanguageSQL;
    $sql_pic = "SELECT ot_dis_NAME  FROM ot_dis WHERE   ot_dis_ID 	='" . $pid . "'";
    $query_pic = wewebQueryDB($coreLanguageSQL, $sql_pic);
    $row_pic = wewebFetchArrayDB($coreLanguageSQL, $query_pic);
    $txt_pic_funtion = $row_pic[0];
    ...
    foreach ($txt as $key => $value) {
        if (!empty($type)) {
            if (ereg($type . '$', $key)) {
                ...
                if (ereg("(" . implode("|", $listChang) . ")", $value)) {
                    $txt = eregi_replace($txtCheck, $txtTo[1], $value);
                } else {
                    $txt = eregi_replace($txtCheck, $txtTo[0], $value);
                }

                if (ereg("นี้คือ", $txt)) {
                    $txt = eregi_replace("นี้คือ", "", $txt);
                }

                $checkTxtSame = array();
                foreach ($txtChange as  $valueNew) {
                    $txt = eregi_replace("^".$valueNew.$valueNew, $valueNew, $txt);
                     $txt = eregi_replace("^".$valueNew." ", $valueNew, $txt);
                }
